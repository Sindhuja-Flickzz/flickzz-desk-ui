...
export class CreateCalendarComponent implements OnInit, OnDestroy {
  ...
  private loadCalendarForEdit(calendarCode: string): void {
    // First try to get from router state (passed from list page)
    const navigation = this.router.getCurrentNavigation();
    const calendar = navigation?.extras?.state?.['calendar'] as CalendarMasterVO;
      
    if (calendar) {
      this.prefillForm(calendar);
    } else {
      // Fallback: fetch from API
      this.calendarService.getCalendarByCode(calendarCode).subscribe({
        next: (response) => {
          const calendarinfo = (response as any).attributes as CalendarMasterVO; // Handle case where response might not have 'object'
          this.prefillForm(calendarinfo);
        },
        error: (err) => {
          this.submitError = 'Failed to load calendar for editing.';
          console.error('Error loading calendar for edit:', err);
        }
      });
    }
  }
  ...
  private prefillForm(calendar: CalendarMasterVO): void {
    ...
    const workingDaysArray = this.workingDaysArray;
    this.daysOfWeek.forEach((day, index) => {
      const isSelected = calendar.workdays?.some(workday => workday.workday === day && workday.isActive) || false;
      workingDaysArray.at(index).setValue(isSelected);
    });

    // Set holidays
    const holidaysArray = this.holidaysArray;
    calendar.holidays?.forEach(holiday => {
      if (holiday.isActive) {
        const holidayGroup = this.createHolidayGroup();
        holidayGroup.patchValue({
          holidayDate: this.dateUtils.formatToYYYYMMDD(new Date(holiday.holidayDate)),
          description: holiday.description
        });
        holidaysArray.push(holidayGroup);
      }
    });

    // Ensure initial loaded data is considered pristine until a user edit occurs.
    this.calendarForm.markAsPristine();
    this.originalFormValue = this.calendarForm.getRawValue();
  ...
  initializeForm(): void {
    ...
    this.calendarForm.valueChanges.subscribe(() => {
      ...
      this.isUpdateEnabled = !this.deepEqual(this.originalFormValue, this.calendarForm.getRawValue());
    });
  ...
  onSubmit(): void {
    ...
    const serviceCall = this.isEditMode
      ? this.calendarService.updateCalendar(this.editingCalendarCode, calendarData)
    ...
    serviceCall.subscribe({
      ...
      error: (error) => {
        this.submitting = false;
        this.submitError = error.error?.description || `Failed to ${this.isEditMode ? 'update' : 'create'} calendar. Please try again.`;
      }
    });
  }
